...
export default function PublishButton({ storyId, runId, vertical }: { storyId: string; runId: string; vertical: string }) {
  ...
  return <button onClick={markPublished} disabled={busy} className="rounded-full bg-ink px-3 py-1 font-mono text-[11px] font-medium text-white disabled:opacity-50">{busy ? "Saving…" : "Mark published"}</button>;
}
